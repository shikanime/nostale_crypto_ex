defmodule NostaleCrypto.Native do
  ...
  @spec world_next(binary(), 0..65535) :: {binary() | nil, binary()}
  def world_next(_raw, _key), do: err()

  @spec world_encrypt(String.t()) :: binary()
  def world_encrypt(_raw), do: err()

  @spec world_session_decrypt(binary()) :: String.t()
  def world_session_decrypt(_raw), do: err()

  @spec world_channel_decrypt(binary(), 0..65535) :: String.t()
  def world_channel_decrypt(_raw, _key), do: err()

  @spec world_channel_unpack(binary()) :: String.t()
  def world_channel_unpack(_raw), do: err()
